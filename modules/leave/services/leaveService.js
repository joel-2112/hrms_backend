...
const assertEmployeeActive = async (employeeId) => {
  const emp = await Employee.findByPk(employeeId, { attributes: ['id', 'status'] });
...
const assertLeaveTypeValid = async (leaveTypeId) => {
  ...
  if (lt.disabled) throw new AppError(`Leave type "${lt.name}" is disabled`, 422);
  return lt;
};
...
const deleteLeavePeriod = async (id) => {
  ...
  const allocationCount = await LeaveAllocation.count({ where: { leavePeriodId: id } });
  if (allocationCount > 0) {
    throw new AppError('Cannot delete — allocations exist for this period', 409);
  }
...
const createLeavePolicy = async (data) => {
  if (!data.name) throw new AppError('name is required', 422);
  if (!data.leaveTypes?.length) throw new AppError('leaveTypes array is required', 422);

  // Validate all leave type IDs exist
  const typeIds = data.leaveTypes.map(t => t.leaveTypeId);
  const types = await LeaveType.findAll({ where: { id: { [Op.in]: typeIds } } });
  ...
  if (exists) throw new AppError(`Leave policy "${data.name}" already exists`, 409);
...
const createLeavePolicyAssignment = async (data) => {
  if (!data.employeeId || !data.leavePolicyId || !data.leavePeriodId || !data.effectiveFrom) {
    throw new AppError('employeeId, leavePolicyId, leavePeriodId and effectiveFrom are required', 422);
  }
  ...
  if (existing) {
    throw new AppError('Employee already has a policy assignment for this period', 409);
  }
...
const generateAllocations = async (assignmentId) => {
  ...
  if (assignment.allocationsGenerated) {
    throw new AppError('Allocations already generated for this assignment', 422);
  }
  ...
  const result = await sequelize.transaction(async (t) => {
    ...
    for (const entry of policy.leaveTypes) {
      const leaveType = await LeaveType.findByPk(entry.leaveTypeId, { transaction: t });
    ...
    await assignment.update({ allocationsGenerated: true, docStatus: 1 }, { transaction: t });
...
const createCompensatoryRequest = async (data, userId) => {
  ...
  if (!leaveType.isCompensatory) {
    throw new AppError(`Leave type "${leaveType.name}" is not a compensatory leave type`, 422);
  }
...
const approveCompensatoryRequest = async (id, approverUserId) => {
  ...
  logger.info('CompensatoryLeaveRequest approved', { id, allocationId: result.allocation.id });
  return result;
};
...
const createLeaveApplication = async (data, userId) => {
  const {
    ...
    isHalfDay, halfDayDate, reason, followUpDate, holidayListId,
  } = data;
  ...
  const totalLeaveDays = await calculateWorkingDays(
    fromDate, toDate,
    leaveType.includeHolidays, leaveType.includeWeekends,
...
const isDateHoliday = async (dateStr, companyId) => {
  ...
  for (const list of lists) {
    ...
    if (isHolidayInList(dateStr, list.holidays)) {
      return { isHoliday: true, holidayListName: list.name, description: list.holidays.find(h => h.date === dateStr)?.description };
    }
  }
