module.exports = (sequelize, DataTypes) => {
  const LeaveApplication = sequelize.define('LeaveApplication', {
    ...
    holidayListId: {
      type:      DataTypes.UUID,
      allowNull: true,
      comment:   'FK → holiday_lists.id — used to exclude holidays from day count',
    },
    ...
    totalLeaveDays: {
      ...
      allowNull: false,
      comment:   'Computed: working days between fromDate and toDate excluding holidays/weekends per LeaveType config',
    },
    ...
    halfDayDate: {
      type:      DataTypes.DATEONLY,
      allowNull: true,
      comment:   'The specific date when half-day is taken — only relevant when isHalfDay = true',
    },
    ...
    // ── Workflow status ────────────────────────────────────────
    status: {
      type:         DataTypes.ENUM('Draft', 'Open', 'Approved', 'Rejected', 'Cancelled'),
      allowNull:    false,
      defaultValue: 'Draft',
    },
    docStatus: {
    ...
    comment:   'Employee leave request — the core transaction of the leave module',
    indexes: [
      ...
      { fields: ['from_date', 'to_date'], name: 'idx_leave_applications_period' },
    ],
  });
