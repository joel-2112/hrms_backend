module.exports = (sequelize, DataTypes) => {
  const LeaveLedgerEntry = sequelize.define('LeaveLedgerEntry', {
    id: {
      type:         DataTypes.UUID,
    ...
    isExpired: {
      ...
      allowNull:    false,
      defaultValue: false,
      comment:      'True when the associated leave period has ended — balance can no longer be used',
    },
  }, {
    tableName: 'leave_ledger_entries',
    comment:   'Immutable audit log — every balance movement writes one row. Current balance = SUM(leaves) per employee per leaveType',
    indexes: [
