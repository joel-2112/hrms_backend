...
const getAllCompanies = async ({ includeDisabled = false, parentCompanyId } = {}) => {
  const where = {};
...
const deleteCompany = async (id) => {
  ...
  if (subsidiaries > 0)
    throw new AppError(`Cannot delete — company has ${subsidiaries} subsidiary(ies)`, 409);
  if (branches > 0)
    throw new AppError(`Cannot delete — company has ${branches} branch(es)`, 409);
  if (departments > 0)
    throw new AppError(`Cannot delete — company has ${departments} department(s)`, 409);
...
const createBranch = async ({ name, companyId }) => {
  ...
  return Branch.create({ name, companyId });
};
...
const deleteBranch = async (id) => {
  ...
  if (empCount > 0)
    throw new AppError(`Cannot delete — ${empCount} employee(s) assigned to this branch`, 409);
...
const getAllDepartments = async ({ companyId, parentDepartmentId, includeDisabled = false } = {}) => {
  const where = {};
  if (!includeDisabled) where.disabled = false;
  if (companyId) where.companyId = companyId;
...
const deleteDepartment = async (id) => {
  ...
  if (subDepts > 0)
    throw new AppError(`Cannot delete — department has ${subDepts} sub-department(s)`, 409);
  if (empCount > 0)
    throw new AppError(`Cannot delete — ${empCount} employee(s) assigned to this department`, 409);
...
const deleteDesignation = async (id) => {
  ...
  if (empCount > 0)
    throw new AppError(`Cannot delete — ${empCount} employee(s) hold this designation`, 409);
...
const deleteEmploymentType = async (id) => {
  ...
  if (empCount > 0)
    throw new AppError(`Cannot delete — ${empCount} employee(s) use this employment type`, 409);
...
const createEmployeeGrade = async ({ name, description, defaultLeavePolicyId }) => {
  const exists = await EmployeeGrade.findOne({ where: { name } });
  if (exists) throw new AppError(`Employee grade "${name}" already exists`, 409);
  return EmployeeGrade.create({ name, description, defaultLeavePolicyId: defaultLeavePolicyId || null });
};
...
const deleteEmployeeGrade = async (id) => {
  ...
  if (empCount > 0)
    throw new AppError(`Cannot delete — ${empCount} employee(s) assigned to this grade`, 409);
