...
const generateReferenceNumber = async (prefix, where = {}) => {
  ...
  const field = prefix === 'REQ' ? 'requisitionNumber' : 'name';
...
/**
 * Submit staffing plan for approval (Draft → Active).
 */
...
/**
 * GM approves requisition (Level 2) - final approval.
 */
const approveGMRequisition = async (id, userId, remarks = null) => {
  ...
  logger.info('JobRequisition GM-approved', { requisitionId: id, gmId: gm.id });
...
const cancelJobRequisition = async (id, userId, remarks = null) => {
  ...
  if (requisition.overallStatus === 'Approved') {
    throw new AppError('Approved requisition cannot be cancelled.', 422);
  }
